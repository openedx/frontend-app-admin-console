...
describe('API functions', () => {
  ...
  describe('getAllRoleAssignments', () => {
    ...
    it('fetches and returns role assignments with correct params', async () => {
      const querySettings = {
        roles: 'course_staff',
        scopes: 'course-v1:OpenedX+DemoX+DemoCourse',
        organizations: 'OpenedX',
        search: 'john',
        order: 'asc',
        sortBy: 'role',
        pageSize: 10,
      ...
      const result = await getAllRoleAssignments(querySettings);
      expect(result.results).toHaveLength(1);
      expect(result.count).toBe(1);
      expect(result.results[0].username).toBe('johndoe');
      expect(getAuthenticatedHttpClient().get).toHaveBeenCalled();
    });
